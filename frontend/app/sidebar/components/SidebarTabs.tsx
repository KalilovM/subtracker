 ...
 */
export function SidebarTabs({ currentPath, previousPath }: SidebarTabsProps) {
	const pathname = usePathname();

	// Create refs for each tab wrapper (spans with relative positioning)
	const tabWrapperRefs = useRef<
		Record<string, React.RefObject<HTMLSpanElement | null>>
	>({});
	TAB_ITEMS.forEach((tab) => {
		if (!tabWrapperRefs.current[tab.href]) {
			tabWrapperRefs.current[tab.href] =
	...
	const activeTabRef =
		tabWrapperRefs.current[currentPath] ||
		tabWrapperRefs.current[Object.keys(tabWrapperRefs.current)[0]];
	const prevTabRef = tabWrapperRefs.current[previousPath || ""] || activeTabRef;

	// Get animated indicator values
	const indicatorStyle = useTabIndicator(activeTabRef, prevTabRef);
